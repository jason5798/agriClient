    ...
    methods: {
      ...
      selectTypeVal (ele) {
        ...
        for (var a = 0; a < types.length; a++) {
          console.log('####  types[' + a + ']: ' + JSON.stringify(types[a]) + 'select name : ' + this.typeName)
      ...
      verifyProfile (isNeedCheckName) {
        ...
        if (profile) {
          ...
          Object.keys(profile.setting).forEach(function (content) {
            ...
            if (profile.setting[content]['max'] !== '' && profile.setting[content]['min'] !== '') {
              if (Number(profile.setting[content]['max']) <= Number(profile.setting[content]['min'])) {
                isError = true
              }
            }
            if (profile.setting[content]['max'] !== '' || profile.setting[content]['min'] !== '') {
